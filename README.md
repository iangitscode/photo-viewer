...
- **Upload race.** The QR code appears while the booth is still uploading, so the page usually loads before the files exist, and the GIF lands several seconds after the strip. Every file is retried when it fails to load, with a backoff from 1 s up to 8 s, for about 70 s in total (12 retries). Retries add `?attempt=N` so a cached error response isn't reused.
